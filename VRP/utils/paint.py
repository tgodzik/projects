...
def draw_route(route, problem, screen):
    ...
    for i in route:
        if current_cargo > problem.customers[i].demand:
            pygame.draw.line(screen, color, (5 * problem.customers[i].x, 5 * problem.customers[i].y),
                             (5 * current_x, 5 * current_y), 2)
        else:
            pygame.draw.line(screen, color, (5 * problem.depotx, 5 * problem.depoty), (5 * current_x, 5 * current_y), 2)
            pygame.draw.line(screen, color, (5 * problem.depotx, 5 * problem.depoty),
                             (5 * problem.customers[i].x, 5 * problem.customers[i].y), 2)
            current_cargo = problem.capacity
